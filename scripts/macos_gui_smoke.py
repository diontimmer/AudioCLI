#!/usr/bin/env python3
"""macOS-first smoke helper for AudioCLI GUI packaging.

Default/static mode is safe in Linux CI and must not import PySide6. Native GUI
construction is opt-in with --construct-window and intended for macOS runners.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--allow-non-darwin",
        action="store_true",
        help="Allow native checks to run outside macOS. Static checks always work anywhere.",
    )
    parser.add_argument(
        "--entrypoint",
...
def run_static_smoke() -> dict[str, Any]:
    ...
    default_scan_specs = macos_default_plugin_scan_directory_specs()
    scan_dirs = default_plugin_scan_directories(platform="darwin", existing_only=False)
    discovered_plugins = discover_default_plugins(platform="darwin")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.construct_window and sys.platform != "darwin" and not args.allow_non_darwin:
        parser.error("--construct-window is native macOS-only unless --allow-non-darwin is set")
    ...
    if args.json:
        ...
        print("AudioCLI GUI macOS smoke")
        ...
        for spec in static["default_plugin_scan_specs"]:
            print(f"- {spec['format']} {spec['scope']}: {spec['path']}")
