...
def test_cli_json_flag_appears_in_help():
    """Every op subcommand exposes a ``--json`` flag.

    Inspect the Click command's registered parameters directly rather than
    rendering the help text — Rich's terminal-width detection on TTY-less CI
    runners truncates flag names with ``…`` regardless of any ``COLUMNS``
    we set, making any string-based help-text assertion unreliable.
    """
    import typer.main  # noqa: PLC0415

    cli = typer.main.get_command(app)
    gain_cmd = cli.get_command(click.Context(cli), "gain")
    flag_names: list[str] = []
    for param in gain_cmd.params:
