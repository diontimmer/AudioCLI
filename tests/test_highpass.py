...
from audiocli.ops.highpass import highpass
...
def test_highpass_round_trip(tmp_path):
    src = _sine(1000.0)
    out = highpass(src, hz=200.0)
...
def test_highpass_attenuates_low_passes_high():
    ...
    low = _sine(100.0)
    high = _sine(10000.0)
    low_out = highpass(low, hz=cutoff)
    high_out = highpass(high, hz=cutoff)
    assert _rms(low_out) < _rms(low) * 0.5
    assert _rms(high_out) > _rms(high) * 0.8


def test_highpass_negative_silent_input_stays_silent():
    """Silent input through any high-pass cutoff must remain silent."""
    silent = AudioBuffer(data=np.zeros((1, SR), dtype=np.float32), sr=SR, subtype="FLOAT")
    out = highpass(silent, hz=1000.0)
